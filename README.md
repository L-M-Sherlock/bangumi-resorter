...
- 快速、标准、精细三档比较预算；几百条收藏的快速模式最多建议 80 次判断
- Worker 内运行带 L2 先验的 Bradley–Terry / MAP 拟合，避免大量条目时阻塞界面
- 均匀、Gwern 反 J、保持原分布、高分辨率尾部、自定义权重五种 1–10 分映射
...
每个条目有一个潜在偏好强度。比较结果的似然由两者强度差的 logistic 函数给出；平局按半胜处理。优化器使用原评分派生的先验和 L2 正则，Newton 外循环配合预条件共轭梯度求解，并用线搜索保证目标函数不下降。快速模式使用更强的原评分先验，优先询问同分作品和相邻分档；标准、精细模式会逐步减弱先验并扩大候选范围。候选问题再按预期信息量、不确定性和重复惩罚选择。
