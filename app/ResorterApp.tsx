...
function LibraryView({ snapshot, items, sessions, onStart, onResume, onSyncAgain }: {
  ...
  async function start() {
    setBusy(true); setError("");
    try { await onStart(selectedType, statuses, distributionConfig(preset, customWeights), budgetMode); }
  ...
  return <>
    ...
    <section className="dashboard-grid">
      ...
      <article className="panel start-panel"><span className="eyebrow">新建排序会话</span><h2>选择本次范围</h2><p>不同媒介会分别建立模型，比较记录可在后续会话中复用。</p>
        ...
        <div className="field-group"><label htmlFor="distribution-preset">新评分分布</label><select id="distribution-preset" value={preset} onChange={(event) => setPreset(event.target.value as DistributionPreset)}><option value="uniform">均匀 1–10</option><option value="reverse-j">反 J 分布（Gwern 推荐）</option><option value="preserve">保持原分布</option><option value="high-tail">高分辨率尾部</option><option value="custom">自定义权重</option></select>{preset === "reverse-j" && <small className="field-help">约 50% 为 1 分、25% 为 2 分，越高分档越稀有；8–10 分各约 1%。</small>}{preset === "custom" && <DistributionWeights weights={customWeights} onChange={setCustomWeights} />}</div>
        {error && <Notice tone="error">{error}</Notice>}
...
function ResultsView({ state, onBack, onDistribution, onExportCsv }: { state: CompareState; onBack: () => void; onDistribution: (distribution: DistributionConfig) => Promise<void>; onExportCsv: (result: RankedItem[]) => void }) {
  ...
  return <>
    ...
    <section className="result-summary"><article className="panel"><div className="panel-title"><div><span className="eyebrow">评分分布对比</span><h2>原评分 → 新评分</h2></div><select id="result-distribution-preset" value={state.session.distribution.preset} onChange={(event) => { const preset = event.target.value as DistributionPreset; void onDistribution(distributionConfig(preset, state.session.distribution.weights)); }}><option value="uniform">均匀 1–10</option><option value="reverse-j">反 J 分布（Gwern 推荐）</option><option value="preserve">保持原分布</option><option value="high-tail">高分辨率尾部</option><option value="custom">自定义权重</option></select></div>{state.session.distribution.preset === "custom" && <DistributionWeights weights={state.session.distribution.weights} onChange={(weights) => void onDistribution({ preset: "custom", weights })} />}<Histogram items={state.items} result={result} /><div className="chart-legend"><span><i className="old" />原评分</span><span><i className="new" />新评分</span></div></article><article className="summary-stat"><span>不确定性较初始降低</span><strong>{Math.round(uncertaintyDrop * 100)}%</strong><small>当前平均值 {state.model.currentMeanUncertainty.toFixed(2)} · 越低越稳定</small><hr /><span>用于本次排序的人工比较</span><strong>{state.model.acceptedComparisons}</strong><small>不含跳过，可能包含同类旧会话记录</small></article></section>
