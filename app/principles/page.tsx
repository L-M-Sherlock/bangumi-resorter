    ...
    <div className="principles-layout">
      ...
      <article className="principles-article">
        ...
        <section>
          ...
          <p><Term term="dynamic-forecast">动态剩余预测</Term>不是把当前进度除以平均速度。每次作答先生成权威后验、停止诊断和下一题；64 路径预测随后在独立后台 Worker 中更新，新模型会取消尚未完成的旧预测。系统从当前后验选取最多 64 个能力与 <code>log ν</code> 联合粒子，并把有限粒子的经验均值精确移到当前 MAP；这个平移保留协方差，却消除模拟从另一套排序中心出发的漂移。每条路径使用实际策略选择普通题、覆盖探索或到期复问，以该路径真值粒子的 Davidson 三结果概率模拟答案，并联合更新能力和平局强度。遇到重复作品对时，更新会用新增答案后的总质量重建整簇相关性折权似然，替换旧簇似然，再做二维局部 Laplace 矩匹配与全图协方差传播；相反答案因此能同时降低旧答案的簇内权重。</p>
