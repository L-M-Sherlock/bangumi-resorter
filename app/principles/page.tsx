    ...
    <div className="principles-layout">
      ...
      <article className="principles-article">
        ...
        <section>
          ...
          <p>纯贪心仍会忽略边缘作品，所以系统混入两类有意的“低效率”：<Term term="coverage-exploration">覆盖探索</Term>把比较带到证据稀少或尚未稳定的区域；<Term term="calibration-repeat">校准复问</Term>交换左右重问旧题，同时提供偏好证据和一致性诊断。所有非跳过答案都会入模，但同一无序作品对构成相关簇：折减后的权重总量 m 按工作相关系数 ρ = 0.5 折算为 m/[1+max(0,m−1)ρ]；整数 m≥1 时就是常见的 m/[1+(m−1)ρ]。所以校准复问和导入的同对历史都不会被当成条件独立的新样本，低于一条完整判断的陈旧权重也不会被反向放大。重复惩罚与覆盖探索同样读取这种来源年龄及相关性修正后的作品对和作品有效权重，不读取原始记录次数。</p>
        </section>
        ...
        <section>
          ...
          <p><Term term="dynamic-forecast">动态剩余预测</Term>不是把当前进度除以平均速度。每次作答先生成权威后验、停止诊断和下一题；桌面端 64 路径、手机端 32 路径的预测随后在独立后台 Worker 中更新，新模型会取消尚未完成的旧预测。系统从当前后验选取与设备预算相同数量的能力与 <code>log ν</code> 联合粒子，并把有限粒子的经验均值精确移到当前 MAP；这个平移保留协方差，却消除模拟从另一套排序中心出发的漂移。会话分析历史检查点固定使用 64 路径，以保持跨设备缓存可比。每条路径使用实际策略选择普通题、覆盖探索或到期复问，以该路径真值粒子的 Davidson 三结果概率模拟答案，并联合更新能力和平局强度。遇到重复作品对时，更新会用新增答案后的总质量重建整簇相关性折权似然，替换旧簇似然，再做二维局部 Laplace 矩匹配与全图协方差传播；相反答案因此能同时降低旧答案的簇内权重。</p>
          <p>局部更新只负责每 16 题的快速筛查，不能授权停止。只有当局部 MC 下界触及 90% 门槛、距离上次完整刷新达到 128 题，或来到预测窗口末尾时，路径才用截至该点的完整历史暖启动 MAP，并重新生成 Laplace 后验；三档停止时刻只能由这个刷新后的全局后验确认。这既避免局部二维近似过早宣告稳定，也不必在每个检查点都付出完整拟合成本。三档共享每一条路径并分别记录首次达标时刻，最后报告 10%、50% 与 90% 分位数，因此快速不晚于标准、标准不晚于精细。</p>
          <p>所以范围可能变宽、变窄甚至暂时上升：新答案可能暴露此前被先验掩盖的不确定性。预测窗口内没有成功路径也不证明目标不可达。它只说明，在当前模型和有限前瞻下，还没有足够证据给出有限上界；大型收藏的前瞻窗口会设上限以保持界面响应。这里显示的是固定算法假设下的情景模拟分位数，不能当作已校准的预测区间。</p>
          <p>会话分析把两种时间分开：证据、覆盖、后验和停止下界按判断实际发生时间重建，导入判断使用 <code>sourceCreatedAt</code>；动态剩余和回溯则按判断真正进入当前会话的 <code>createdAt</code> 重建，避免让后来导入的证据穿越到当时的选题器。停止只在离散检查点观察，所以覆盖与误差按相邻检查点之间的停止窗口计算；导入或手动判断改变未来可用证据时，相应预测不参与评分。至少三个闭合回溯区间中若 80% 同方向漏窗，且 P50 中位偏差达到一个检查点步长，页面会警告并抑制该档历史 P10–P90 阴影，但不会用同一会话的相关检查点强行校准未来倍数。</p>
        </section>
