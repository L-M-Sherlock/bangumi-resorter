...
test("principles page is public and terminology popovers are keyboard accessible", async ({ page }) => {
  ...
  const modelPopover = page.getByRole("dialog", { name: "Davidson 成对比较模型解释" });
  await expect(modelPopover).toBeVisible();
  await expect(modelPopover).toContainText("潜在分数差和平局强度");
  ...
  await expect(page.locator(".technical-box")).toContainText("ν 是带弱正则的共享平局强度。模型用 MAP 估计寻找最符合比较与先验的一组 θ 和 ν");
